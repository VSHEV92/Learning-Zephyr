...
void thread_sender(void * arg_1, void * arg_2, void * arg_3) {
    while(true) {
        for(int i = 0; i < DATA_ITEMS_NUMBER; i++) {
            data_items[i].data = i;
            k_queue_append	(&queue, &data_items[i]);
        }
...
void thread_receiver(void * arg_1, void * arg_2, void * arg_3) {
    ...
    while(true) {
        data_item = (struct data_item_t*) k_queue_peek_head(&queue);
        if (data_item) {
            printf("Receiver thread peek head data: %d\n", data_item->data);
            k_queue_remove(&queue, data_item);
        }
    }
}
