...
// work item delay. Reschedule update work dalay time, so work will not ever get to queue
void threads_entry_1(void * arg_1, void * arg_2, void * arg_3) {
    while(true) {
        k_work_reschedule(&work_1, K_MSEC(1000));
        k_msleep( 500 );
    }
}
...
void threads_entry_2(void * arg_1, void * arg_2, void * arg_3) {
    while(true) {
        k_work_reschedule(&work_2, K_MSEC(500));
        k_msleep( 1000 );
    }
}
